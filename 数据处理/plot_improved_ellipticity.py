...
# -*- coding: utf-8 -*-
"""
plot_improved_ellipticity.py - 绘制改进的克尔椭率数据图
对于实验4和5的数据，x需要变成-x，然后进行中心平移处理。
"""
...
try:
    from plot_modified_data import load_experiment_data, modify_data, plot_modified_experiments, setup_chinese_font
...
def invert_x_for_experiments(experiment_data: List[List[List[float]]], 
                            experiment_indices: List[int]) -> List[List[List[float]]]:
    """
    对指定实验的x数据进行取反操作（x → -x）和中心对称变换
    
    参数:
    experiment_data (list): 原始实验数据列表
    experiment_indices (list): 需要取反的实验索引列表（从0开始）
    ...
    for i, experiment in enumerate(experiment_data):
        if len(experiment) != 2:
            print(f"警告: 实验 {i+1} 数据格式错误，保持原样")
            modified_data.append(experiment)
            continue
        
        x_data, y_data = experiment
        
        # 如果当前实验在需要取反的列表中，则对x取反并进行中心对称变换
        if i in experiment_indices:
            # 对x数据取反
            x_inverted = [-x for x in x_data]
            
            # 尝试导入find_centre中的parity_transform函数
            try:
                ...
                # 进行中心对称变换
                x_transformed, y_transformed = parity_transform(x_inverted, y_data, x_c=0, y_c=y_centre)
                
                modified_data.append([x_transformed.tolist(), y_transformed.tolist()])
                print(f"实验 {i+1}: x数据已取反并进行中心对称变换")
                print(f"  原x范围: [{min(x_data):.2f}, {max(x_data):.2f}]")
                print(f"  取反后x范围: [{min(x_inverted):.2f}, {max(x_inverted):.2f}]")
                print(f"  变换后x范围: [{min(x_transformed):.2f}, {max(x_transformed):.2f}]")
                print(f"  对称中心: (0, {y_centre:.6f})")
                
            except ImportError as e:
                print(f"警告: 无法导入find_centre模块: {e}")
                print(f"实验 {i+1}: 仅进行x取反，未进行中心对称变换")
                modified_data.append([x_inverted, y_data])
            except Exception as e:
            ...
            modified_data.append(experiment)
            print(f"实验 {i+1}: x数据保持原样")
    
    return modified_data


def process_improved_ellipticity_data() -> Tuple[List[np.ndarray], List[np.ndarray], List[float]]:
    """
    处理改进的克尔椭率数据：对实验4和5的x取反，然后进行中心平移
    ...
    if len(kerr_ellipticity_data) < 5:
        print(f"警告: 克尔椭率数据不足，期望5个，实际{len(kerr_ellipticity_data)}个")
    
    # 3. 对实验4和5的x数据进行取反（索引3和4，从0开始）
    print("\n对实验4和5的x数据进行取反操作...")
    experiment_indices_to_invert = [3, 4]  # 实验4和5的索引（从0开始）
    improved_data = invert_x_for_experiments(kerr_ellipticity_data, experiment_indices_to_invert)
    
    # 4. 进行中心平移处理
    print("\n进行中心平移处理...")
    if MODULES_AVAILABLE:
        # 使用plot_modified_data中的modify_data函数
        modified_x_list, modified_y_list, centres = modify_data(improved_data)
    else:
        # 自定义处理
        modified_x_list = []
        modified_y_list = []
        ...
        for i, experiment in enumerate(improved_data):
            if len(experiment) == 2:
                x_data, y_data = experiment
...
def plot_improved_ellipticity(modified_x_list: List[np.ndarray],
    ...
    for i, (x_data, y_data) in enumerate(zip(modified_x_list, modified_y_list)):
        ...
        if len(y_data) >= 7:
            ...
            # 绘制max7均值横线（实线）
            ax.axhline(y=max7_mean, color=color, linestyle='-', linewidth=1.5, alpha=0.8,
                      label=f'实验 {i+1} max7均值')
            
            # 绘制min7均值横线（实线）
            ax.axhline(y=min7_mean, color=color, linestyle='-', linewidth=1.5, alpha=0.8,
    ...
    # 设置图形属性
    title = "改进的克尔椭率数据对比 (实验4&5 x取反+中心对称变换, y中心平移到0)"
    ax.set_title(title, fontsize=16, pad=20)
    ax.set_xlabel("磁感应强度 (mT)", fontsize=14)
    ax.set_ylabel("克尔椭率 (度, 中心修正)", fontsize=14)
    ...
    # 保存图形
    output_path = "数据处理/克尔椭率_improved.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"改进的克尔椭率图已保存到: {output_path}")
    
    # 打印处理信息
    print("\n处理摘要:")
    print("-" * 40)
    print("实验1-3: x数据保持原样")
    print("实验4-5: x数据已取反并进行中心对称变换")
    ...
    # 保存改进处理信息（包含max7和min7均值表格）
    save_improvement_info(centres, max7_means, min7_means)
    
    # 保存改进数据到JSON文件
    save_improved_data_to_json(modified_x_list, modified_y_list, centres, max7_means, min7_means)
    
    # 显示图形（放在最后，避免阻塞后续代码执行）
    plt.show()
...
def save_improvement_info(centres: List[float], 
    ...
    """
    output_path = "数据处理/ellipticity_info.txt"
    
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write("克尔椭率数据改进处理信息\n")
            f.write("=" * 50 + "\n\n")
            
            f.write("处理说明:\n")
            f.write("- 实验1-3: x数据保持原样\n")
            f.write("- 实验4-5: x数据已取反并进行中心对称变换\n")
            f.write("- 所有实验: y中心值已平移到0\n\n")
...
def save_improved_data_to_json(modified_x_list: List[np.ndarray],
    ...
    min7_means (list): 每个实验的min7均值列表
    """
    output_path = "数据处理/improved_ellipticity_data.json"
...
def plot_3d_hysteresis_slices(modified_x_list: List[np.ndarray],
                             modified_y_list: List[np.ndarray],
                             centres: List[float]) -> None:
    """
    绘制3D磁滞回线切片图
    ...
    参数:
    modified_x_list (list): 修正后的x数据列表（磁感应强度）
    modified_y_list (list): 修正后的y数据列表（中心平移到0的克尔椭率值）
    centres (list): 每个实验的原始中心值列表（作为z坐标）
    """
    if not modified_x_list or not modified_y_list or not centres:
        print("错误: 没有有效数据可绘制3D图")
        return
    
    if len(modified_x_list) != len(centres):
        print(f"错误: 数据长度不匹配，x数据:{len(modified_x_list)}，中心值:{len(centres)}")
        return
    ...
    for i, (x_data, y_data) in enumerate(zip(modified_x_list, modified_y_list)):
        ...
        # 创建z坐标数组（所有点的z值都等于该实验的中心值）
        z_data = np.full_like(x_data, centres[i])
        
        # 绘制3D散点图
        ax.scatter(x_data, y_data, z_data, color=color, s=30, alpha=0.7, 
                   label=f'实验 {i+1} (z={centres[i]:.3f})', edgecolors='black', linewidth=0.5)
        
        # 绘制3D连线（形成闭合磁滞回线）
        # 创建闭合的数据：将第一个点添加到末尾
        x_closed = np.append(x_data, x_data[0])
        y_closed = np.append(y_data, y_data[0])
        z_closed = np.append(z_data, z_data[0])
        
        ax.plot(x_closed, y_closed, z_closed, color=color, linestyle='--', linewidth=1.5, alpha=0.8)
    
    # 设置图形属性
    ax.set_title("改进克尔椭率的3D磁滞回线切片图", fontsize=16, pad=20)
    ax.set_xlabel("磁感应强度 (mT)", fontsize=12, labelpad=10)
    ax.set_ylabel("克尔椭率 (度, 中心修正)", fontsize=12, labelpad=10)
    ax.set_zlabel("原始中心值 (度)", fontsize=12, labelpad=10)
    
    # 设置视角
    ax.view_init(elev=25, azim=45)
    
    # 添加图例
    ax.legend(loc='upper left', fontsize=10, bbox_to_anchor=(0, 0.85))
    ...
    x_min, x_max = all_x.min(), all_x.max()
    y_min, y_max = all_y.min(), all_y.max()
    z_min, z_max = min(centres), max(centres)
    ...
    # 保存图形
    output_path = "数据处理/克尔椭率_3d_slices.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"3D磁滞回线切片图已保存到: {output_path}")
    ...
    print("-" * 50)
    print("x轴: 磁感应强度 (mT)")
    print("y轴: 修正后的克尔椭率值 (中心已平移到0)")
    print("z轴: 原始中心值 (每个实验的y中心值)")
    print("\n各实验在z轴的位置:")
    for i, centre in enumerate(centres):
        print(f"  实验 {i+1}: z = {centre:.6f} 度")


def main():
    """主函数"""
    print("开始绘制改进的克尔椭率数据图...")
    print("="*60)
    
    # 1. 处理改进的克尔椭率数据
    modified_x_list, modified_y_list, centres = process_improved_ellipticity_data()
    ...
    # 2. 绘制改进的克尔椭率数据图（2D）
    plot_improved_ellipticity(modified_x_list, modified_y_list, centres)
    
    # 3. 注释掉3D磁滞回线切片图的绘制（根据用户要求取消3D图功能）
    # plot_3d_hysteresis_slices(modified_x_list, modified_y_list, centres)
    
    print("\n" + "="*60)
    print("改进的克尔椭率数据图绘制完成")
    print("="*60)
